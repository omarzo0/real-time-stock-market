...
const Alert = ({ children, className }) => (
  <div
    className={`bg-red-900 border border-red-700 text-white p-4 rounded-lg ${className}`}
  >
    {children}
  </div>
);
...
const IndexCard = ({ title, value, change, Icon }) => (
  <Card className="bg-gray-800 border-gray-700">
    <div className="flex flex-row items-center justify-between space-y-0 pb-2">
      ...
      <Icon
        className={`h-4 w-4 ${change >= 0 ? "text-green-400" : "text-red-400"}`}
      />
    </div>
    <div>
  ...
  return (
    <div className="min-h-screen bg-black text-white p-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-8">
        <IndexCard
          title="S&P 500"
          value={stockData.SP500.value}
          change={stockData.SP500.change}
          Icon={TrendingUp}
        />
        <IndexCard
          title="NASDAQ"
          value={stockData.NASDAQ.value}
          change={stockData.NASDAQ.change}
          Icon={TrendingDown}
        />
        <IndexCard
          title="Dow Jones"
          value={stockData.DOW.value}
          change={stockData.DOW.change}
          Icon={Activity}
        />
      </div>
